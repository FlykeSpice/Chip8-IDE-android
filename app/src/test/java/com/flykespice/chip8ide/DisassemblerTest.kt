...
//Maze rom
val rom = intArrayOf(0x12,0x5,0x43,0x38,0x50,0x60,0x0,0x85,0x0,0x60,0x1,0x81,0x50,0xA3,0xE6,0xF1,0x1E,0xF0,0x55,0x60,
...
class DisassemblerTest {
    ...
    @Test
    fun disassemble() {
        val disasm = Disassembler.disassemble(rom)

        val assembled = Assembler.assemble(disasm).getOrNull()!!

        for(i in assembled.indices) {
            val byte = assembled[i]
            if(byte != rom[i]) {
                println("Difference at byte ${(0x200+i).toString(16)}: ${byte.toString(16).padStart(2,'0')} <> ${rom[i].toString(16).padStart(2,'0')}")
            }
        }
    }
}
